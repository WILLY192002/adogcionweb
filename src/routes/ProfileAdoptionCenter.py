...
def viewProfileAdoptionCenter(id,name):
  if request.method == 'POST':
    if current_user.is_authenticated and current_user.get_id() == id and UsertypeService.verifyUserTypeAdoptionCenter(current_user.user_type_id):
      ...
      adoption_center_department = request.form.get('department').capitalize() if request.form.get('department') != None else None
      adoption_center_city = request.form.get('city').capitalize() if request.form.get('city') != None else None
